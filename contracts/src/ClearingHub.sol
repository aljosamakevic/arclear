...
/// @title ClearingHub — multilateral obligation netting for one ERC-20
/// @notice Participants accumulate signed off-chain IOUs, then settle only net
///         positions from pre-posted collateral in a single atomic round.
///
...
///         (unanimous consent). A coordinator may assemble rounds but holds no
///         authority: it cannot forge consent, and `executeRound` is
///         permissionless given the signatures.
///
///         Trust model (v1): IOUs never touch the chain; each round commits
///         `manifestHash` — the keccak256 of the sorted list of consumed IOU
///         ids — so which paper a round extinguished is provable after the
///         fact. Counterparty exposure between rounds is bounded off-chain by
///         bilateral credit caps. No owner access to funds, no upgradeability,
///         no fees; `pause` gates deposits and rounds but never withdrawal.
contract ClearingHub is EIP712, Ownable2Step, Pausable, ReentrancyGuard {
